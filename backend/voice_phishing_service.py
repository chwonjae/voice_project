...
INITIAL_OPENING_TEXT = "잠시만! 채영아 입금 전에 하나 확인하려고 해. 지금 큰 금액을 입금하는데 어디에 입금하는 거야?"
GEMINI_MODEL = "gemini-3-flash-preview"
ELEVENLABS_VOICE_ID = "XrExE9yKIg1WjnnlVkGX"
ELEVENLABS_MODEL_ID = "eleven_flash_v2_5"
SUPPORTED_WAV_CONTENT_TYPES = {"audio/wav", "audio/x-wav"}
...
class TurnSessionStartResponse(BaseModel):
    session_id: str
    intro_message: str
    intro_audio_base64: str
    conversation_status: ConversationStatus
    risk_level: RiskLevel
    risk_score: int
...
class TurnReplyResponse(BaseModel):
    user_transcript: str
    system_message: str
    audio_base64: str
    risk_level: RiskLevel
    risk_score: int
    suspected_types: list[SuspectedType]
...
@dataclass
class TurnChatSession:
    session_id: str
    chat: Any
    history: list[dict[str, str]] = field(default_factory=list)
    latest_result: Optional[VoicePhishingOutput] = None
...
class VoicePhishingService:
    def __init__(self) -> None:
        self._sessions: dict[str, TurnChatSession] = {}
    ...
    def create_session(self) -> TurnSessionStartResponse:
        session_id = uuid.uuid4().hex
        chat = self._create_chat()
        intro_audio = self._synthesize_speech_base64(INITIAL_OPENING_TEXT)

        self._sessions[session_id] = TurnChatSession(
            session_id=session_id,
            chat=chat,
            history=[{"role": "assistant", "text": INITIAL_OPENING_TEXT}],
        )

        return TurnSessionStartResponse(
            session_id=session_id,
            intro_message=INITIAL_OPENING_TEXT,
            intro_audio_base64=intro_audio,
    ...
    def reply_to_turn(
        ...
        if not audio_bytes:
            raise HTTPException(
                ...
                detail="업로드된 음성 데이터가 비어 있습니다.",
            )

        if content_type and content_type not in SUPPORTED_WAV_CONTENT_TYPES:
            raise HTTPException(
                status_code=status.HTTP_415_UNSUPPORTED_MEDIA_TYPE,
                detail="WAV 형식의 음성만 지원합니다.",
            )

        transcript = self._transcribe_wav_bytes(audio_bytes)
        if not transcript:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        parsed = self._generate_turn_result(session.chat, transcript)
        audio_base64 = self._safe_synthesize_speech_base64(parsed.system_message)

        session.history.append({"role": "user", "text": transcript})
        session.history.append({"role": "assistant", "text": parsed.system_message})
        session.latest_result = parsed
        session.terminated = parsed.conversation_status == "terminated"

        return TurnReplyResponse(
            user_transcript=transcript,
            system_message=parsed.system_message,
            audio_base64=audio_base64,
            risk_level=parsed.risk_level,
            risk_score=parsed.risk_score,
            suspected_types=list(parsed.suspected_types),
            key_evidence=list(parsed.key_evidence),
            immediate_action=list(parsed.immediate_action),
            next_question=parsed.next_question,
            conversation_status=parsed.conversation_status,
    ...
    def _get_session_or_404(self, session_id: str) -> TurnChatSession:
    ...
    def _transcribe_wav_bytes(self, audio_bytes: bytes) -> str:
        sample_rate_hertz, channel_count = self._read_wav_metadata(audio_bytes)
        response = self._get_speech_client().recognize(
    ...
    def _read_wav_metadata(self, audio_bytes: bytes) -> tuple[int, int]:
        try:
    ...
    def _generate_turn_result(self, chat: Any, transcript: str) -> VoicePhishingOutput:
        try:
            response = chat.send_message(transcript)
            if getattr(response, "parsed", None) is not None:
                return VoicePhishingOutput.model_validate(response.parsed)
            return VoicePhishingOutput.model_validate_json(response.text)
        except ValidationError as exc:
            raise HTTPException(
                status_code=status.HTTP_502_BAD_GATEWAY,
                detail="LLM 응답을 스키마로 해석하지 못했습니다.",
            ) from exc
        except Exception as exc:
            raise HTTPException(
                status_code=status.HTTP_502_BAD_GATEWAY,
                detail=f"LLM 응답 생성에 실패했습니다: {exc}",
            ) from exc

    def _safe_synthesize_speech_base64(self, text: str) -> str:
        try:
            return self._synthesize_speech_base64(text)
        except Exception:
            return ""

    def _synthesize_speech_base64(self, text: str) -> str:
        audio_stream = self._get_elevenlabs_client().text_to_speech.convert(
            text=text,
        ...
        if self._elevenlabs_client is None:
            ...
            if not api_key:
                raise HTTPException(
                    ...
                    detail="ELEVENLABS_API_KEY 환경 변수가 설정되어 있지 않습니다.",
                )
...
voice_phishing_service = VoicePhishingService()
